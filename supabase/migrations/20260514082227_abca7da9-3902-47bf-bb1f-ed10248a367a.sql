ALTER TABLE public.stays ADD COLUMN IF NOT EXISTS is_permanent BOOLEAN NOT NULL DEFAULT false;

DROP FUNCTION IF EXISTS public.get_stays_with_privacy(text, uuid);
  ...
  SELECT 
    ...
    CASE 
      WHEN (s.user_id IS NOT NULL AND s.user_id = _viewer_id)
        ...
        OR (s.user_id IS NOT NULL AND _viewer_id IS NOT NULL AND has_mutual_connection(_viewer_id, s.user_id))
        OR (s.user_id IS NOT NULL AND _viewer_id IS NOT NULL AND has_approved_reveal(_viewer_id, s.user_id))
        OR (_viewer_id IS NOT NULL AND s.status = 'confirmed' AND EXISTS (
              SELECT 1 FROM stays vs WHERE vs.village_id = s.village_id AND vs.user_id = _viewer_id AND vs.status = 'confirmed'))
      THEN s.nickname ELSE 'Anonymous' END AS nickname,
    s.villa,
    s.start_date,
    s.end_date,
    CASE 
      WHEN (s.user_id IS NOT NULL AND s.user_id = _viewer_id)
        ...
        OR (_viewer_id IS NOT NULL AND s.status = 'confirmed' AND EXISTS (
              SELECT 1 FROM stays vs WHERE vs.village_id = s.village_id AND vs.user_id = _viewer_id AND vs.status = 'confirmed'))
      THEN s.intention ELSE NULL END AS intention,
    CASE 
      WHEN (s.user_id IS NOT NULL AND s.user_id = _viewer_id)
        OR (_viewer_id IS NOT NULL AND is_village_host(_viewer_id, s.village_id))
        OR (s.user_id IS NOT NULL AND _viewer_id IS NOT NULL AND has_mutual_connection(_viewer_id, s.user_id))
        ...
        OR (_viewer_id IS NOT NULL AND s.status = 'confirmed' AND EXISTS (
              SELECT 1 FROM stays vs WHERE vs.village_id = s.village_id AND vs.user_id = _viewer_id AND vs.status = 'confirmed'))
      THEN s.social_profile ELSE NULL END AS social_profile,
    CASE 
      WHEN (s.user_id IS NOT NULL AND s.user_id = _viewer_id)
        OR (_viewer_id IS NOT NULL AND is_village_host(_viewer_id, s.village_id))
        OR (s.user_id IS NOT NULL AND _viewer_id IS NOT NULL AND has_mutual_connection(_viewer_id, s.user_id))
        ...
        OR (_viewer_id IS NOT NULL AND s.status = 'confirmed' AND EXISTS (
              SELECT 1 FROM stays vs WHERE vs.village_id = s.village_id AND vs.user_id = _viewer_id AND vs.status = 'confirmed'))
      THEN s.offerings ELSE NULL END AS offerings,
    CASE 
      WHEN (s.user_id IS NOT NULL AND s.user_id = _viewer_id)
        OR (_viewer_id IS NOT NULL AND is_village_host(_viewer_id, s.village_id))
        OR (s.user_id IS NOT NULL AND _viewer_id IS NOT NULL AND has_mutual_connection(_viewer_id, s.user_id))
        ...
        OR (_viewer_id IS NOT NULL AND s.status = 'confirmed' AND EXISTS (
              SELECT 1 FROM stays vs WHERE vs.village_id = s.village_id AND vs.user_id = _viewer_id AND vs.status = 'confirmed'))
      THEN s.asks ELSE NULL END AS asks,
    s.secret_hash,
    s.is_host,
    s.created_at,
    CASE 
      WHEN (s.user_id IS NOT NULL AND s.user_id = _viewer_id)
        ...
        OR (_viewer_id IS NOT NULL AND s.status = 'confirmed' AND EXISTS (
              SELECT 1 FROM stays vs WHERE vs.village_id = s.village_id AND vs.user_id = _viewer_id AND vs.status = 'confirmed'))
      THEN s.project_description ELSE NULL END AS project_description,
    CASE 
      WHEN (s.user_id IS NOT NULL AND s.user_id = _viewer_id)
        OR (_viewer_id IS NOT NULL AND is_village_host(_viewer_id, s.village_id))
        OR (s.user_id IS NOT NULL AND _viewer_id IS NOT NULL AND has_mutual_connection(_viewer_id, s.user_id))
        ...
        OR (_viewer_id IS NOT NULL AND s.status = 'confirmed' AND EXISTS (
              SELECT 1 FROM stays vs WHERE vs.village_id = s.village_id AND vs.user_id = _viewer_id AND vs.status = 'confirmed'))
      THEN s.project_url ELSE NULL END AS project_url,
    s.status,
    CASE 
      WHEN (s.user_id IS NOT NULL AND s.user_id = _viewer_id)
        OR (_viewer_id IS NOT NULL AND is_village_host(_viewer_id, s.village_id))
